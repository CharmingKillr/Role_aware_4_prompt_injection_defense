    ...
    while len(completed) < len(output_dirs):
        for output_dir in [x for x in output_dirs if x not in completed]:
            ...
            for attack in args.test_attack:
                ...
                base_cmd = f'python test_my.py --model_name_or_path {output_dir} --attack {attack} --defense {args.defense} --data_path {args.test_data}'
                #base_cmd = f'python test.py --model_name_or_path {output_dir} --attack {attack} --defense {args.defense} --data_path {args.test_data}'
                ...
                if args.print_cmd: print(cmd + '\n' * 5)
                    ...
                    if attack == 'gcg': 
                        ...
                        for i in range(len(to_run_sample_ids) // num_gcg_sample_per_gpu + 1):
                            ...
                            cmd = base_cmd + f' --sample_ids {sample_ids}'
                            print("\n" + "=" * 80)
                            print(f"[INFO] Testing attack: {attack} on {output_dir}")
                            print(f"[INFO] Sample IDs: {sample_ids}")
                            print(f"[INFO] CMD:\n{cmd}")
                            print("=" * 80 + "\n")
                            ret = os.system(cmd)
                            if ret != 0:
                                print(f"[WARN] command for testing attack {attack} on {output_dir} "
                                    f"with sample_ids [{sample_ids}] exited with code {ret}")
                            time.sleep(2)
                            #time.sleep(1)
                    else:
                        # with open(temporary_slurm_file, 'w') as f: f.write(slurm_prefix + cmd)
        ...
        if test_waiting_time > 100 * 60 * 60: exit() # kill the python script after 100 hours
