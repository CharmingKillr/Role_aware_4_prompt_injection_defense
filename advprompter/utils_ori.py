...
def check_jailbroken(seq, test_prefixes):
    ...
    jailbroken_list = [any([prefix in text for prefix in test_prefixes]) for text in seq.text] # reverse it for prompt injection
...
def llm_loader(llm_params, verbose=False):
    ...
    cfg = AutoConfig.from_pretrained(llm_params.checkpoint, trust_remote_code=True)

    # 你训练的 role 模型一般会把 num_roles 写进 config；如果没写，也可以用名字兜底
    is_role_ckpt = (getattr(cfg, "num_roles", None) is not None) or ("Role" in llm_params.checkpoint)

    ModelCls = AutoModelForCausalLM
    if is_role_ckpt:
        if cfg.model_type == "llama":
            ModelCls = LlamaForCausalLMWithRole
        elif cfg.model_type == "mistral":
            ModelCls = MistralForCausalLMWithRole
    ...
    if llm_params.lora_params is not None:
        if llm_params.lora_params.lora_checkpoint is not None:
            tqdm.write(
        ...
        elif llm_params.lora_params.lora_config is not None:
