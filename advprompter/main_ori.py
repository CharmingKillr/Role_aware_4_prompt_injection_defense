...
class Workspace:
    ...
    def eval_suffix_dataset(self, suffix_dataset_key, suffix_dataset_pth):
        ...
        instruct_af_dict = defaultdict(list)
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            _, affirmative_list = check_affirmative(
                seq=target_llm_ar.response_sample, affirmative_prefixes=self.affirmative_prefixes
            )
            ...
            for i in range(instruct.bs):
                ...
                instruct_af_dict[instruct_text[i]].append(affirmative_list[i])
            # -----------
        ...
        af_all = [af_list for (instruct, af_list) in instruct_af_dict.items()]
        max_length = max(len(sublist) for sublist in af_all)
        padded_list = [
            np.pad(sublist, (0, max_length - len(sublist)), "constant")
            for sublist in af_all
        ]
        af_stat_np = np.array(padded_list)

        for ti in range(1, jb_stat_np.shape[1] + 1):
            tqdm.write(
                f"{suffix_dataset_key} | in-response rate @ {ti}: {hit_rate_at_n(jb_stat_np, ti)}"
            )
        for ti in range(1, af_stat_np.shape[1] + 1):
            tqdm.write(
                f"{suffix_dataset_key} | begin-with rate @ {ti}: {hit_rate_at_n(af_stat_np, ti)}"
            )
